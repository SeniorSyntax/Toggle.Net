...
public class RegisterToggledComponentTest(bool toggleState, bool resolveOnce)
{
	...
	private class enableToggledRegistrations(IServiceProvider serviceProvider) : IToggledRegistrationPicker
	{
		public object PickService<TOn, TOff>(string toggleName)
		{
			if(serviceProvider.GetService<ToggleState>().IsEnabled(toggleName))
				return serviceProvider.GetService<MyServiceOn>();
			return serviceProvider.GetService<MyServiceOff>();
		}
	}
	...
	public void BeforeTest()
	{
		...
		serviceCollection.AddSingleton<MyServiceOn>();
		serviceCollection.AddSingleton<MyServiceOff>();
		serviceCollection.RegisterToggledComponent<MyServiceOn, MyServiceOff, IMyService>(theToggle, resolveOnce);
		serviceCollection.EnableToggledRegistrations<enableToggledRegistrations>();
		serviceCollection.AddSingleton<ToggleState>();
		_serviceProvider = serviceCollection.BuildServiceProvider();
		_myService = _serviceProvider.GetService<IMyService>();
		_toggleState = _serviceProvider.GetService<ToggleState>();
		_toggleState.Set(theToggle, toggleState);
	}

	[TearDown]
	public void AfterTest() => _serviceProvider.Dispose();

	[Test]
	public void ShouldHaveProxyAsSingleInstance()
	{
		using var newScope = _serviceProvider.GetService<IServiceScopeFactory>().CreateScope();
		var myService2 = newScope.ServiceProvider.GetService<IMyService>();
	...
	public void ClassProxiesNotSupportedAtTheMoment()
	{
		...
		{
			serviceCollection.RegisterToggledComponent<MyServiceOn, MyServiceOff, object>(theToggle, resolveOnce);
		});
	}
